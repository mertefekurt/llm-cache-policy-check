...
from llm_cache_policy_check import __version__
from llm_cache_policy_check.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from llm_cache_policy_check.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='llm-cache-policy-check',
        description=(
                        'Check LLM response cache policies for privacy, TTL, and key-scope gaps.'
                    ),
    )
    parser.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    parser.add_argument("input", nargs="?", type=Path, help='LLM cache policy')
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"llm-cache-policy-check: error: {exc}", file=sys.stderr)
        return 1
